...
// configuration step. It creates a namespace called `workflow`. You can modify
...
{
  try {
    CLI::App app{ fmt::format("{} version {}", workflow::cmake::project_name, workflow::cmake::project_version) };
    ...
    if (show_version) {
      fmt::print("{}\n", workflow::cmake::project_version);
      return EXIT_SUCCESS;
    }
